...
const ProductTable = ({ product, fetchLoad, deleteProductHandle, viewProductHandle }) => {
   ...
   return (
      <>
         {
            fetchLoad ? <Spinner></Spinner> :
               <>
                  <h6>
                     {
                        product.length <= 0 ? "No product Found" : product.length === 1 ? "Total " + product.length + " Product found" : "Total " + product.length + " Products found"
                     }
                  </h6>
                  <br />
                  <div className="table-responsive product_table" style={{ maxHeight: "80vh", overflowY: "auto" }}>
